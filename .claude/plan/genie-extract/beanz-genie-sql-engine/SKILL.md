...
description: >
  ...
  analysis, shipment SLA, roaster MOT, cancellation survey, email performance, CVM, open rates,
  click rates, send jobs, BIEDM campaigns, welcome series, email engagement, or any analytics
  question that touches the Beanz data warehouse. Also triggers when building dashboards, reports,
  slide decks, or strategic analyses that need verified Beanz numbers. Even if the user just says
  "pull the data", "check the numbers", "query genie", "what does genie say", "get me the latest
  figures", "email results", or "how are our emails performing" — this skill applies. Do NOT skip
  this skill for any BeanzGenie interaction.
---
...
### Rule 11: Email Performance Queries (factemailevents + dimsendjobs)

Email event data lives in `ana_prd_gold.edw.factemailevents` (events) joined to
`ana_prd_gold.edw.dimsendjobs` (send metadata). There is no dedicated CVM schema.

**Mandatory filters for ALL email queries:**
1. `e.BrandRegionPartition = 'Beanz'` — table is cross-brand (Breville, Sage, Baratza, etc.)
2. Use partition columns for date filtering: `e.YearPartition = 2026 AND e.MonthPartition = 3`
3. `TRIM(sj.EmailBrand) = 'Beanz'` when querying dimsendjobs directly (trailing space on some rows)

**Always deduplicate for rates:**
- Raw event counts inflate because one subscriber can open/click multiple times
- Use `COUNT(DISTINCT CASE WHEN e.EventType = 'Sent' THEN e.SubscriberKey END)` for unique rates
- Apple Mail Privacy Protection inflates opens — unique open rates will exceed 80% for triggered emails
...
**EmailRegion caveat:** Most journey/triggered emails (Welcome Series, Order Confirmation, Shipment, etc.)
have `NULL` EmailRegion in dimsendjobs. Only BIEDMs and some regional campaigns carry AU/US/UK/DE tags.
The NULL bucket is the largest by volume.

**Category mapping:** Use the `CASE WHEN sj.EmailName LIKE 'BIEDM%' THEN ...` pattern in
`references/schema-reference.md` Section 19 for consistent email category grouping.
...
| Email event performance (sends, opens, clicks, bounces, unsubs) | `factemailevents` | dimsendjobs (SendID), dimbeanzcustomeremail (SubscriberKey=CustomerEmail). **ALWAYS filter `BrandRegionPartition = 'Beanz'`**. Use partition columns (YearPartition, MonthPartition) for date filtering. Deduplicate with `COUNT(DISTINCT SubscriberKey)`. |
| Email send metadata (name, subject, region, flags) | `dimsendjobs` | factemailevents (SendID). **Filter `TRIM(EmailBrand) = 'Beanz'`** (trailing space). |
...
All 19 tables live in schema: `ana_prd_gold.edw`
...
- [ ] If querying email data: `BrandRegionPartition = 'Beanz'` present AND using partition columns for dates
- [ ] If querying dimsendjobs: `TRIM(EmailBrand) = 'Beanz'` present (trailing space)
- [ ] If calculating email rates: using `COUNT(DISTINCT SubscriberKey)` not raw `COUNT(*)`
...
- [ ] If email query: `BrandRegionPartition = 'Beanz'` applied (without it, includes all Breville brands)
- [ ] If email query: rates use `COUNT(DISTINCT SubscriberKey)` not raw counts (without dedup, opens inflate 1.5–3x)
- [ ] If email query: `TRIM(EmailBrand)` used on dimsendjobs (trailing space causes misses)
...
| Email: Beanz subscribers in factemailevents | 394,100 unique | ±5% | BrandRegionPartition='Beanz', all-time |
| Email: Total Beanz event rows | 19.3M | ±5% | All-time, all event types |
| Email: BIEDM unique open rate (CY26 Q1) | ~44–53% (varies by region) | ±5% | UK highest (~50–53%), AU/US ~44–47% |
| Email: Triggered email unique open rate | ~80–90% | ±5% | Transactional/lifecycle emails; Apple MPP inflates |
| Email: BIEDM unique CTR | ~0.8–2.8% | ±30% | Highly variable by campaign content |
| Email: Subscription Lifecycle CTR (CY26 Q1) | ~46.7% | ±5% | Upcoming Subscription + Payment Failure drive this |
| Email: Transactional CTR (CY26 Q1) | ~40.2% | ±5% | Shipment tracking links drive high clicks |
| Email: Welcome Series CTR (CY26 Q1) | ~12.5% | ±10% | Down from ~17% in CY25 Q1 — investigate |
| Email: FTBPv2 converters (CY26 YTD) | 9,271 unique | ±5% | Conv_7Day_Beanz_FTBPv2='true'. Attribution caveat applies. |
