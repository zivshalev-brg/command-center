...
function renderTechNewsMain() {
  ...
  if (DATA.techNewsError) {
    el.innerHTML = '<div class="ca-main" style="padding:var(--sp4)">'
      ...
      +   '<div class="c-empty-title" style="color:var(--rd)">Failed to load tech news</div>'
      +   '<div class="c-empty-body">' + _tnEnc(DATA.techNewsError) + '</div>'
      +   '<button class="c-btn c-btn-primary c-empty-action" onclick="loadTechNewsData()">Retry</button>'
      + '</div></div>';
    return;
  }
  if (!DATA.techNews || !DATA.techNews.articles) {
    el.innerHTML = '<div class="ca-main" style="padding:var(--sp4)">'
      + '<div class="c-empty">'
      +   '<div class="c-empty-icon">\uD83D\uDCF0</div>'
      +   '<div class="c-empty-title">No tech news yet</div>'
      +   '<div class="c-empty-body">Click Refresh to pull from YouTube, Reddit, industry press and podcasts.</div>'
      +   '<button class="c-btn c-btn-primary c-empty-action" onclick="loadTechNewsData()">Refresh feed</button>'
      + '</div></div>';
    return;
  }
  ...
  html += '<div class="nw-stats">' +
    ...
    _tnStatChip((stats.byCategory || {}).podcast || 0, 'Podcasts') +
    '</div>';

  // Articles
  var sectionLabel = state.techNewsCategory === 'podcast' ? 'Podcast episodes (' + totalCount + ')' : 'Articles (' + totalCount + ')';
  html += '<div class="nw-section-label">' + sectionLabel + '</div>';

  if (!pageArticles.length) {
    if (state.techNewsCategory === 'podcast') {
      html += '<div class="c-empty" style="padding:var(--sp6)">'
        + '<div class="c-empty-icon">\uD83C\uDFA7</div>'
        + '<div class="c-empty-title">No AI/tech podcasts configured yet</div>'
        + '<div class="c-empty-body">Add YouTube channels of your favourite AI podcasts (Lex Fridman, Dwarkesh, Latent Space, Cognitive Revolution, ML Street Talk). Click <b>Manage Sources</b> to paste a YouTube channel ID under <b>Podcasts</b>.</div>'
        + '<button class="btn btn-p btn-sm" style="margin-top:var(--sp3)" onclick="state.techNewsCategory=\'settings\';renderAll()">\u2192 Manage Sources</button>'
        + '</div>';
    } else {
      html += '<div class="c-empty" style="padding:var(--sp6)">'
        + '<div class="c-empty-icon">\uD83D\uDD0D</div>'
...
function renderTechNewsBriefing() {
  ...
  if (d.error || !d.digest) return '<div class="ca-main"><div class="ca-header"><h2>Daily Briefing</h2></div>'
    ...
    +   '<button class="c-btn c-btn-primary c-empty-action" onclick="loadTechNewsDigest(\'daily\')">Retry</button>'
    + '</div></div>';
...
function renderTechNewsResearch() {
  ...
  if (rpt.podcast_highlights && rpt.podcast_highlights.length) {
    ...
    rpt.podcast_highlights.forEach(function(p) {
      ...
      if (thumb) {
        html += '<a href="' + _tnEnc(url) + '" target="_blank" rel="noopener" style="display:block;border-radius:8px;overflow:hidden;align-self:start;position:relative">' +
      ...
      html += '<div>';
      if (p.show) html += '<div style="font-size:10px;color:var(--gn);text-transform:uppercase;letter-spacing:1px;font-weight:var(--fw-sb);margin-bottom:4px">' + _tnEnc(p.show) + '</div>';
      html += '<h3 style="font-size:var(--f-lg);margin:0 0 var(--sp2);color:var(--tx)"><a href="' + _tnEnc(url) + '" target="_blank" rel="noopener" style="color:var(--tx);text-decoration:none">' + _tnEnc(p.episode_title || 'Episode') + '</a></h3>';
      if (p.host_summary) html += '<div style="font-size:var(--f-sm);line-height:1.65;color:var(--tx2);margin-bottom:var(--sp3)">' + _tnEnc(p.host_summary) + '</div>';
      if (p.key_segments && p.key_segments.length) {
        p.key_segments.forEach(function(seg) {
          ...
          html += '<div style="margin:var(--sp2) 0;padding:8px 12px;background:var(--s2);border-left:3px solid var(--gn);border-radius:0 6px 6px 0">' +
            (seg.topic ? '<div style="font-size:11px;color:var(--gn);font-weight:var(--fw-sb);margin-bottom:2px">' + _tnEnc(seg.topic) + (ts ? ' · [' + ts + ']' : '') + '</div>' : '') +
            '<div style="font-style:italic;font-size:var(--f-sm);color:var(--tx)">"' + _tnEnc(seg.quote||'') + '"</div>' +
            (seg.speaker ? '<div style="font-size:10px;color:var(--tx3);margin-top:4px;display:flex;justify-content:space-between"><span>— ' + _tnEnc(seg.speaker) + '</span><a href="' + _tnEnc(segUrl) + '" target="_blank" rel="noopener" style="color:var(--gn)">▶ Listen</a></div>' : '') +
          '</div>';
        });
      }
      if (p.takeaway) html += '<div style="font-size:var(--f-sm);color:var(--gn);margin-top:var(--sp2)"><strong>Takeaway:</strong> ' + _tnEnc(p.takeaway) + '</div>';
      html += '</div></div>';
    });
  }
